...
fun provideThornedCrawlerTriggers(balance: JsonObject): Map<String, SbTrigger> = mapOf(
    "thorned_crawler.vicious_pincers" to { context, event ->
        context.useMonsterAbility(event, THORNED_CRAWLER_VICIOUS_PINCERS) { characterId, koef ->
    ...
    "thorned_crawler.leeching_shadows" to { context, event ->
        context.useMonsterAbility(event, THORNED_CRAWLER_LEECHING_SHADOWS) { characterId, koef ->
            val character = game.character(characterId) ?: return@useMonsterAbility
            val damage = SbElemental(
